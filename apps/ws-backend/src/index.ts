...
import { JWT_SECRET } from "@repo/backend-auth";
	...
	ws.on("message", async (data: any) => {
		...
		try {
			...
			if (message.type === "join-room") {
				...
				const authResult = await canUserJoinRoom(
					Number(user.userId),
					roomId.toString(),
				);
				...
				console.log(
					"all users after join:",
					users.map((u) => ({ id: u.userId, rooms: u.rooms })),
				);
			}
			...
			if (message.type === "draw") {
				...
				await prisma.drawing.create({
					data: {
						...
						points: message.element.points
							? JSON.stringify(message.element.points)
							: null,
						userId: Number(user.userId),
						roomId: room.id,
					},
				...
				console.log(
					"users in room:",
					users.filter((u) => u.rooms.includes(roomSlug)).length,
				);

				console.log("draw received from user:", user.userId);
				console.log(
					"broadcasting to N users:",
					users.filter((u) => u.rooms.includes(roomSlug)).length,
				);
				// broadcasting
