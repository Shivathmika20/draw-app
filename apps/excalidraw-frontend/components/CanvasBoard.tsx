...
function CanvasBoard({ tool, roomId }: Prop) {
  ...
  // console.log('onMessage type:', typeof onMessage) 
	...
	useEffect(() => {
		...
		// subscribe once nd keep on getting info
		const unsub = onMessage((msg) => {
      console.log("canvas received msg:", msg.type, JSON.stringify(msg))
	...
	const handleMouseMove = (e: React.MouseEvent) => {
		...
		// Update shape being drawn — broadcast live update
		setElements((prev) => {
			const copy = [...prev];
			...
			// broadcast live so others see it drawing in real time
			send({ 
        type: "update", 
        element: { 
            ...last, 
	...
	const handleMouseUp = () => {
		// broadcast final position of moved shape
  ...
  useEffect(() => {
	...
	// Draw canvas evrytime element chnages
	useEffect(() => {
    console.log('redrawing, elements count:', elements.length)  
		const canvas = canvasRef.current;
